...
//		m_Rgn->Attach(hr);
		...
		if(hr != NULL)
		{
			if (!::SetWindowRgn (GetSafeHwnd(), hr, TRUE)) //(HRGN) *m_Rgn, TRUE))
				MessageBox ("Region Oluþturulamadý", MsgCaption, MB_OK);
			DeleteObject(hr);
		}
		else
			MessageBox ("Resimden Region Oluþturulamadý", MsgCaption, MB_OK);
		Invalidate ();
	}
}
...
void CSkinDialog::SetSkinFile(char *mSkinName)
{
...
void CSkinDialog::ReadTrackBarInfo(CIniFile m_File)
{
	...
	for(int i = 0; i < m_SlidersInfo.GetSize(); i++)
	{
		...
		if (!mBtnName.IsEmpty())
		{
			...
			m_Slider->Create(WS_VISIBLE, r, this, 0);
