...
// Altered by William Chin
// on the 8th June 2000
...
INT     CALLBACK AppAbout( HWND, UINT, WPARAM, LPARAM );
...
int ext;
char title_bar_string[256];
char strFileExt[256];
char strFileName[256];
char strSkinMapFileName[256];
char filename[256];
char strFile[1024];
char strTitle[1024];
char strFileTitle[1024];
char strInitialDir[1024];	
char szLoadFilter[1024];
char szSaveFilter[1024];

char *LoadFilterString[] = {"Quake2 (*.MD2)","*.MD2"};


BOOL bIsOkToAnimate = TRUE;
...
HRESULT CD3DApplication::Create( HINSTANCE hInst, TCHAR* strCmdLine )
{
	...
	memset(&open_filename, 0, sizeof(OPENFILENAME));
	open_filename.lStructSize = sizeof(OPENFILENAME);

	open_filename.hwndOwner = m_hWnd; 
	open_filename.lpstrFile = strFile;
	open_filename.nMaxFile = 1024;
	open_filename.lpstrFileTitle = strFileTitle;
	open_filename.nMaxFileTitle = 1024;

	memset(&szLoadFilter, 0, sizeof(char));

	cnt = 0;

	for(i = 0; i < 2; i++)
	{
		strcpy(&szLoadFilter[cnt], LoadFilterString[i]);
		cnt += strlen(LoadFilterString[i])+1;
		szLoadFilter[cnt] = 0;
	}
                                  ...
                                  LPARAM lParam )
{
    ...
    {
		case WM_CREATE:
			SetTimer(hWnd, ANIMATION_TIMER, 150,NULL);
			break;

		case WM_TIMER: 
			if(wParam == ANIMATION_TIMER)
			{
				if(bIsOkToAnimate == TRUE)
				{
					model.CurrentFrame++;	
        ...
        case WM_COMMAND:
            switch( LOWORD(wParam) )
            {
				...
				case ID_ANIMATION_STOP:
					bIsOkToAnimate = FALSE;
					break;

				case ID_ANIMATION_NEXTFRAME:
					if(bIsOkToAnimate == FALSE)
					{
						model.CurrentFrame++;	
						if(model.CurrentFrame >= model.NumFrames)
							model.CurrentFrame = 0;
				...
				case IDM_OPEN:
					int i;			
					ext = 0;
					open_filename.lpstrFilter = &szLoadFilter[0];
					GetOpenFileName(&open_filename);
					
					// Search through filename for '.'
					// and remove file extension
					for(i = 0; i < 256; i++)
					{
						// Search for '.'
						if(strFileTitle[i] == 46)
						{
							// Set end of string here
							strFileName[i] = 0;
							break;
						}	
						// copy filename without extention
						strFileName[i] = strFileTitle[i];
					}	
					strcpy(strFileExt, &strFileTitle[i+1]);
				
					if(strcmpi(strFileExt,"MD2") == 0) 
					{
						D3DTextr_InvalidateAllTextures();
						pCMD2->ReleaseMD2Model(&model);
						pCMD2->LoadMD2Model(&model, strFile);
						D3DTextr_RestoreAllTextures(m_pd3dDevice);
					
						strcpy(title_bar_string, "MD2 View v1.0 - ");
						strcat(title_bar_string, strFileName);
						strcat(title_bar_string, ".");
						strcat(title_bar_string, strFileExt);
						SetWindowText(hWnd, title_bar_string);
					}
					break;
...
VOID CD3DApplication::ShowStats()
{
	...
	// Output animation frame number to screen
    sprintf( buffer, " Frame %d", model.CurrentFrame); 
    OutputText( 0, 20, buffer );

	// Output frame name to screen
	sprintf( buffer, " Frame name %s", model.Frames[model.CurrentFrame].framename); 
